...
export default function EditWorkoutTemplateModal({
  visible,
  onClose,
  onSuccess,
  template,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient.put(`/api/workout-templates/${template.id}`, payload);
      ...
      Alert.alert("Error", "Failed to update workout template");
    } finally {
      setLoading(false);
    }
  };
        ...
        <View style={styles.reorderButtons}>
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              index === selectedExercises.length - 1 && styles.disabled,
            ]}
          >
            <Text style={styles.reorderText}>↓</Text>
          </TouchableOpacity>
          ...
          {selectedExercises.length === 0 ? (
            ...
            <>
              <Text style={styles.reorderHint}>
                Use ↑↓ buttons to reorder exercises
              </Text>
              {selectedExercises.map((ex, idx) =>
                renderSelectedExercise(ex, idx),
              )}
            </>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  disabled: { backgroundColor: "#9ADAD6", opacity: 0.5 },
